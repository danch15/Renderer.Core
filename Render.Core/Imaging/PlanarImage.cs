...
namespace Renderer.Core.Imaging
{
    ...
    public unsafe sealed class PlanarImage : ISerializable, IDisposable, ICloneable
    {
            ...
            : this(width, height, pixelType)
        {
            ...
            if (NumberOfPlanes > 1)
            {
                IntPtr intPtr1 = pixelType == PixelAlignmentType.I420 ? pixelData + PlaneSizes[0] + PlaneSizes[1] : pixelData + PlaneSizes[0];
                IntPtr intPtr2 = pixelType == PixelAlignmentType.I420 ? pixelData + PlaneSizes[0] : pixelData + PlaneSizes[0] + PlaneSizes[1];
                RtlMoveMemory(Planes[1], intPtr1, PlaneSizes[1]);
                if (NumberOfPlanes > 2)
                {
                    RtlMoveMemory(Planes[2], intPtr2, PlaneSizes[2]);
                }
            }
        }
