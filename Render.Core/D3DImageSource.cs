...
namespace Renderer.Core
{
    public class D3DImageSource : IRenderSource
    {
        ...
        private void FillBuffer(IntPtr bufferPtr)
        {
            ...
            {
                case FrameFormat.YU12: //I420就是存储方式为Planar，抽样方式为4:2:0，数据组成为YYYYYYYYUUVV的一种色彩编码格式
                    #region 填充I420数据
                    if (rect.Pitch == this.yStride)
                    {
                        Interop.Memcpy(surfaceBufferPtr, bufferPtr, this.ySize);
                        IntPtr intPtrV = bufferPtr + this.ySize; //源数据的V其实位置
                        IntPtr intPtrU = intPtrV + this.uvSize; //源数据的U其实位置
                        Interop.Memcpy(surfaceBufferPtr, intPtrU, this.uvSize);
                        Interop.Memcpy(surfaceBufferPtr, intPtrV, this.uvSize);
                    }
                    else
                    {
                        ...
                        IntPtr intPtrV = bufferPtr + this.ySize; //源数据的V其实位置
                        IntPtr intPtrU = intPtrV + this.uvSize; //源数据的U其实位置
                        int vHeight = yHeight / 2;
                        int uHeight = yHeight / 2;

                        int uvPitch = yPitch >> 1;
                        for (int i = 0; i < uHeight; i++) //处理U
                        {
                            Interop.Memcpy(surfaceBufferPtr, intPtrU, this.uvStride);
                            surfaceBufferPtr += uvPitch;
                            intPtrU += this.uvStride;
                        }

                        for (int i = 0; i < vHeight; i++) //处理V
                        {
                            Interop.Memcpy(surfaceBufferPtr, intPtrV, this.uvStride);
                            surfaceBufferPtr += uvPitch;
                            intPtrV += this.uvStride;
                        }
                    }
                    #endregion
                    break;

                case FrameFormat.YV12: //YV12的数据组成：YYYYYYYYVVUU
                    #region 填充YV12数据
                    if (rect.Pitch == this.yStride)
                    {
                ...
                case FrameFormat.NV12: //NV12的数据组成：YYYYYYYYUVUV
